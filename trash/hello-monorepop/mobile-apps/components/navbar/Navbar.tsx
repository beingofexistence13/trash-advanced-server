...
// Main
export default function Navbar(props: { navigationProp: { navigate: (arg0: string) => void; }; headerTittle: string | number | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | React.ReactFragment | React.ReactPortal | null | undefined; }) {
    ...
    let onHandlerStateChange = (event: { nativeEvent: { state: number; }; }) => {
        if (event.nativeEvent.state === State.ACTIVE) {
            alert("Your Holding This for so long");
        }
    };
    let onSingleTap = (event: { nativeEvent: { state: number; }; }) => {
        if (event.nativeEvent.state === State.ACTIVE) {
            alert("I'm touched");
        }
    };
    let onDoubleTap = (event: { nativeEvent: { state: number; }; }) => {
        if (event.nativeEvent.state === State.ACTIVE) {
            alert('Double tap!');
        }
    };
